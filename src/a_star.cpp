...
void A_star::map_callback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
  ...
  init[0] = floor((ix - origin_x) / res);
  init[1] = floor((iy - origin_y) / res);
  goal[0] = floor((gx - origin_x) / res);
  goal[1] = floor((gy - origin_y) / res);
...
bool A_star::search_path(void)
{
  ...
  std::vector<std::vector<bool> > closed(map_row, std::vector<bool>(map_col, false));
  closed[init[0]][init[1]] = true;
  std::vector<std::vector<char> > action(map_row, std::vector<char>(map_col, -1));
  ...
  while(!found && !resign){
    if(!open.size()){
      ...
      std::sort(open.begin(), open.end(),
          [](const Open x, const Open y){return x.f > y.f;});
      next = open.back();
      open.pop_back();
      x = next.x;
      y = next.y;
      ...
      if(x == goal[0] && y == goal[1]){
        ...
        for(int i = 0; i < delta.size(); i++){
          x2 = x + delta[i][0];
          y2 = y + delta[i][1];
          if(x2 >= 0 && x2 < map_row && y2 >= 0 && y2 < map_col){
